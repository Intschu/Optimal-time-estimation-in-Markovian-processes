...
function transition_time(escape_rate::Float64) # sample time to jump out of state with escape rate (escape_rate)
    return -log(rand())/escape_rate
end

function transition_position(jump_rates::Vector{Float64}) # sample next post-jump state
    # jump_rates[j] is a vector of rates from current state to j
    r = rand()
...
function ring_network(n::Int64, transition_rates::Matrix{Float64}, visible_weights::Matrix{Int64}, runs::Int64)
    ...
    current = fcscumulants_recursive(H, J, [sqrt(transition_rates[1, n])*transition(b, 1, n), sqrt(transition_rates[n, 1])*transition(b, n, 1)], 2, steady_state, iterative=:false)
    times = [0.0, 0.0]
...
#visible_weights[1, 5] = 1; visible_weights[5, 1] = 1; visible_weights[2, 3] = 1; visible_weights[3, 2] = 1
#visible_weights[6, 7] = 1; visible_weights[7, 6] = 1;
visible_weights[2, 5] = 1; visible_weights[5, 2] = 1
...
outfile_dwell = "C:\\Users\\Kaces\\Downloads\\ring_hmm_dwell.txt"
outfile_wait = "C:\\Users\\Kaces\\Downloads\\ring_hmm_wait.txt"
